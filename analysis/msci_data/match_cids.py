...
def fuzzy_match_df_cols(left_df, right_df, 
	...
	df[rc] = df.apply(score_filter, axis=1, col=rc, min_score=min_score)
...
def match_files_main(source_file, source_col,
					 dest_file, dest_col,
					 local_out = 'match_key.csv'):



		#Fuzzy Match Leadiro Company Names and Employer Key Company Names
		fuzzy_match_df_cols(source_file, dest_file,
							source_col, dest_col,
							min_score=90,
							outfile=local_out)
...
def prep_files(source_file, dest_file):

	#Prepare Source
	df = pd.read_csv(source_file)
	df = df[['company']]
	print(df.shape)
	df.to_csv("fortune1000_prepped.csv", index=False)

	#Prepare Dest
	df = pd.read_csv(dest_file, dtype=object)
	df = df[['CompanyName']].drop_duplicates()
	print(df.shape)
	df.to_csv("msci_prepped.csv", index=False)



#Prepare Source/Dest Files
source_file = "fortune1000-list.csv"
source_col = "company"
dest_file = "MSCI_ALL_1991_2016.csv"
dest_col = "CompanyName"

prep_files(source_file, dest_file)


#Attempt to Match Files
source_file = "fortune1000_prepped.csv"
source_col = "company"
dest_file = "msci_prepped.csv"
dest_col = "CompanyName"

match_files_main(source_file, source_col, dest_file, dest_col)
